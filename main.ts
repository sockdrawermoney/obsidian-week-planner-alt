...
export default class WeekPlannerPlugin extends Plugin {
    ...
    /**
     * Utility function to extract tags from task content
     * @param taskContent The content of the task line
     * @returns An array of tag names without the '#' prefix
     */
    extractTags(taskContent: string): string[] {
        const tagRegex = /#(\w+(?:\/\w+)*)/g;
        const tags: string[] = [];
        let match;
        while ((match = tagRegex.exec(taskContent)) !== null) {
            tags.push(match[1]); // Capture the tag name without '#'
        }
        return tags;
    }
    ...
    async moveTask(editor: Editor) {
        ...
        if (taskContent.startsWith(TODO_PREFIX) || taskContent.startsWith(TODO_DONE_PREFIX)) {
            ...
            const sourceFileName = activeFile.path; // Use the absolute path directly
            ...
            // Extract tags from the task content
            const tags = this.extractTags(taskContent);

            if (tags.length === 1) {
                // Exactly one tag
                const tagName = tags[0];
                new TodoModal(this.app, tagName, async (dateOrTag: Date | string) => {
                    if (typeof dateOrTag === 'string' && dateOrTag.startsWith('#')) {
                        const tag = dateOrTag.slice(1); // Remove the '#' prefix
                        await this.moveTaskToTag(editor, source, taskContent, tag);
                    } else if (dateOrTag instanceof Date) {
                ...
                // Open modal for user to choose date or tag
                new TodoModal(this.app, undefined, async (dateOrTag: Date | string) => {
                    if (typeof dateOrTag === 'string' && dateOrTag.startsWith('#')) {
                        const tag = dateOrTag.slice(1); // Remove the '#' prefix
                        await this.moveTaskToTag(editor, source, taskContent, tag);
                    } else if (dateOrTag instanceof Date) {
    ...
    async moveTaskToTag(
        ...
        // Remove the '#' from the tag if present
        const tagName = tag.startsWith('#') ? tag.slice(1) : tag;

        // Sanitize the tag name by replacing '/' with space or another safe character
        const sanitizedTagName = tagName.replace(/\//g, ' ').trim();

        // Get or create the tag page
        let tagPageFile = await this.getOrCreateTagPage(sanitizedTagName);
        ...
        if (tagPageFile) {
            ...
            new Notice(`Failed to create or find tag page for #${sanitizedTagName}`);
        }
    }
